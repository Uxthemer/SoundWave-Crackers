...
export function QuickPurchase() {
  const { addToCart, totalQuantity, totalAmount } = useCartStore();
    ...
    <>
      <div className="min-h-screen pt-24 pb-12">
        <div className="container mx-auto px-4">
          <div className="sticky top-[72px] z-40 bg-background/95 backdrop-blur-sm py-4 border-b border-card-border/10 shadow-sm">
            <div className="max-w-6xl mx-auto">
              <h1 className="font-heading text-3xl md:text-4xl mb-4 md:mb-0">Quick Purchase</h1>
              <div className="flex flex-wrap items-center gap-4 bg-card/50 p-4 rounded-xl">
                <div className="flex-1 min-w-[120px] text-center">
                  <p className="text-sm text-text/60">Total Items</p>
          ...
          <div className="mt-8 max-w-6xl mx-auto">
            {Object.entries(groupedProducts).map(([category, categoryProducts]) => (
              <div key={category} className="mb-8">
                <h2 className="font-montserrat font-bold text-xl md:text-2xl mb-4 pl-4 border-l-4 border-primary-orange">
                  {category}
                </h2>
                <div className="space-y-4">
                  {categoryProducts.map((product) => (
                    <motion.div
                      ...
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      className="bg-card/30 rounded-xl p-4 shadow-sm hover:bg-card/50 transition-colors"
                    >
                        ...
                        <div className="flex items-center gap-4 flex-1">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex flex-wrap gap-2 items-center mt-1">
                              <span className="text-sm text-text/60">{product.content}</span>
                              <span className="bg-primary-orange/10 text-primary-orange px-2 py-0.5 rounded-full text-sm">
